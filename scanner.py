...
UNISWAP_V2_ROUTER = "0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24"  # V2 Router
UNISWAP_V3_ROUTER = "0x2626664c2603336E57B271c5C0b26F421741e481"  # V3 SwapRouter
UNISWAP_UNIVERSAL_ROUTER = "0x3fC91A3afd70395Cd496C647d5a6CC9D4B2b7FAD"  # Universal Router
UNISWAP_V4_POOL_MANAGER = "0xC2aB7dD270D16e6C64Dc33fb99eD888aEdE5e623"  # V4 PoolManager on Base
UNISWAP_V2_FACTORY = "0x8909Dc15e40173Ff4699343b6eB8132c65e18eC6"  # V2 Factory

# Minimal ABI for V2 pair contract to check reserves
PAIR_ABI = [
...
def has_liquidity(token_address):
    """Check if the token has liquidity on Uniswap V2 (proxy for new tokens)."""
    ...
    if pair_address == "0x0000000000000000000000000000000000000000":
        print(f"No V2 liquidity pair found for {token_address}")
        return False
    ...
    try:
        ...
        has_liq = reserve > 0
        print(f"Token {token_address} has V2 liquidity: {has_liq} (reserve: {reserve})")
        return has_liq
    except Exception as e:
        print(f"Error checking V2 liquidity for {token_address}: {e}")
        return False  # Assume no liquidity if check fails
...
def process_block(block_number):
    ...
    for tx in block.transactions:
        if tx.to and tx.input.hex().startswith("0x095ea7b3"):  # approve() signature
            ...
            # Check if spender is a Uniswap contract (V2, V3, Universal, or V4)
            uniswap_spenders = [
            ...
            if has_liquidity(token_address):
                print(f"Token {token_address} already has V2 liquidity, skipping")
                continue
            ...
            if token_info:
                ...
                message += f"**Note:** No liquidity detected yet on Uniswap V2"
